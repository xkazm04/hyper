...
export enum LeonardoPreset {
  ...
  PRO_BW_PHOTOGRAPHY = 'Pro B&W photography',
  PRO_COLOR_PHOTOGRAPHY = 'Pro color photography',
  CINEMATIC = 'Cinematic',
  PORTRAIT_FASHION = 'Portrait Fashion',
  SKETCH_BW = 'Sketch (B&W)',
  SKETCH_COLOR = 'Sketch (Color)',
  ILLUSTRATION = 'Illustration',
  GRAPHIC_DESIGN_3D = 'Graphic Design 3D',
}
...
export const PRESET_IDS: Record<LeonardoPreset, string> = {
  ...
  [LeonardoPreset.PRO_BW_PHOTOGRAPHY]: '22a9a7d2-2166-4d86-80ff-22e2643adbcf',
  [LeonardoPreset.PRO_COLOR_PHOTOGRAPHY]: '7c3f932b-a572-47cb-9b9b-f20211e63b5b',
  [LeonardoPreset.CINEMATIC]: 'a5632c7c-ddbb-4e2f-ba34-8456ab3ac436',
  [LeonardoPreset.PORTRAIT_FASHION]: '0d34f8e1-46d4-428f-8ddd-4b11811fa7c9',
  ...
  [LeonardoPreset.GRAPHIC_DESIGN_3D]: '7d7c2bc5-4b12-4ac3-81a9-630057e9e89f',
}
...
export class LeonardoService {
  ...
  private resolveModelId(model?: string): string {
    ...
    for (const [key, value] of Object.entries(LeonardoModel)) {
      if (value === model) {
   ...
   */
  private buildV1Request(request: LeonardoGenerateRequest): Record<string, unknown> {
   ...
   */
  private buildV2Request(request: LeonardoGenerateRequest): Record<string, unknown> {
    const { width, height } = this.normalizeDimensions(
      request.width || 512,
      request.height || 512
    )
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        if (generatedImages.length > 0) {
          return generatedImages.map((img: { url: string; id: string; nsfw?: boolean }) => ({
            url: img.url,
            id: img.id,
  ...
  private async pollGenerationV2(
    ...
    throw new Error(`V2 generation timed out after ${maxAttempts * pollInterval / 1000} seconds`)
  }
  ...
  async generateImages(request: LeonardoGenerateRequest): Promise<LeonardoGenerateResponse> {
    ...
    const payload = useV2 ? this.buildV2Request(request) : this.buildV1Request(request)
    ...
    if (useV2) {
      generationId = generateData.generation?.id || generateData.generationId || generateData.id
    } else {
