...
interface AudioNarrationPanelProps {
  ...
  content: string
  /** Story stack ID for storage path */
  storyStackId: string
...
export function AudioNarrationPanel({
  ...
  }, [content, storyStackId, cardId, isGenerating, onAudioUrlChange])
  ...
  return (
    <div className="mt-2">
      ...
      <div className={cn(
        ...
        {/* Error message */}
        {error && (
          <span className="ml-auto text-xs text-destructive truncate max-w-[200px]" title={error}>
            {error}
          </span>
        )}
      </div>
