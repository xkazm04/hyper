...
export function PublishStatus({ canPublish, isPublished, error }: PublishStatusProps) {
  return (
    <>
      {/* Cannot publish warning */}
      {!canPublish && !isPublished && (
        <div
          className="flex items-start gap-3 p-4 bg-amber-50 dark:bg-amber-950/30
                     border-2 border-amber-300 dark:border-amber-700 rounded-lg"
          data-testid="publish-status-warning"
        >
      ...
      {error && (
        <div
          className="flex items-start gap-3 p-4 bg-red-50 dark:bg-red-950/30
                     border-2 border-red-300 dark:border-red-700 rounded-lg"
          data-testid="publish-status-error"
        >
